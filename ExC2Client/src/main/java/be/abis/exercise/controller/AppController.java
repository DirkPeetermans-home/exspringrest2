...
public class AppController {
	...
	public String findPersonById(@Valid @ModelAttribute("person1") Person person1, BindingResult result){
		Person personFound;
		try {
			personFound = trainingService.findPerson(person1.getPersonId());
	...
	public String removePersonById(@Valid @ModelAttribute("person") Person person,BindingResult result){
		...
		} catch (PersonCanNotBeDeletedException | PersonNotFoundException e) {
			 result.reject("personId", "person could not be deleted");
			 return "removeperson";
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
